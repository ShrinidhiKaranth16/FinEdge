...
describe('POST /users', () => {
  ...
  test('creates user successfully and does not return password', async () => {
    const payload = { name: 'Sanath', email: 'sanath@example.com', password: 'secret123' };
    ...
    // verify persisted and password is hashed
    const users = await readUsers();
    expect(users.length).toBe(1);
    expect(users[0].email).toBe(payload.email);
    expect(users[0].password).toBeDefined();
    expect(users[0].password).not.toBe(payload.password); // should be hashed
  });

  test('returns 409 when email already exists', async () => {
    const payload = { name: 'First', email: 'dup@example.com', password: 'secret123' };
